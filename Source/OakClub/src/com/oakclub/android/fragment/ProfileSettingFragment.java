...
public class ProfileSettingFragment {
		...
		public void onClick(View v) {
			...
			case R.id.user_avatar:
				...
				activity.startActivityForResult(Intent.createChooser(
						...
						activity.getResources().getString(
								R.string.txt_complete_action_photo)),
			...
			case R.id.profileLayoutSchool:
				...
				solveChangeField(Constants.PROFILE_SETTING_FIELD_SCHOOL,
						profileInfoObj.getSchool(), profileSchool.getText()
								.toString());
				break;
			...
			case R.id.profileLayoutEthnicity:
				...
				solveChangeBySingleChoice2(
						...
						getEthnicityArrayListString(),
						getEthnicityArrayListId(),
			...
			case R.id.profileLayoutRelationStatus:
				...
				solveChangeBySingleChoice2(
						...
						getRelationshipArrayListString(),
						getRelationshipArrayListId(),
			...
			public void onClick(View arg0) {
				...
				if (editable != null && editable.length() != 0
					...
					SetProfileByFieldLoader loader = new SetProfileByFieldLoader(
							"setField", activity, fieldName, editable);
	...
	private void solveChangeBySingleChoice(final String fieldName,
		...
		final String[] stringList = activity.getResources().getStringArray(
				stringArrayId);
		...
		for (int i = 0; i < stringList.length; i++) {
			RadioButtonCustom radio = new RadioButtonCustom(activity, activity
					.getResources()
					.getDrawable(R.drawable.radiogroup_selector2));
		...
		okBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (selectedId != -1 && selectedId != currentValue) {
					hasChange = true;
					SetProfileByFieldLoader loader = new SetProfileByFieldLoader(
							"setField", activity, fieldName, "" + selectedId);
					activity.getRequestQueue().addRequest(loader);
	...
	private void solveChangeBySingleChoice2(final String fieldName,
			...
			final ArrayList<Integer> idList, final int currentId,
			final String title) {
		final ArrayList<RadioButton> radioButtons;
		AlertDialog.Builder builder;
		builder = new AlertDialog.Builder(activity);
	...
	private void solveChangeLanguage(final String fieldName,
		...
		for (int i = 0; i < stringList.size(); i++) {
			CheckBoxCustom cb = new CheckBoxCustom(activity, activity
					.getResources().getDrawable(R.drawable.radiogroup_selector));
		...
		okBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				for (int i = 0; i < checkboxList.size(); i++) {
					if (checkboxList.get(i).isChecked()) {
						...
						if (str.length() != 0) {
							str = str
									+ ","
									+ activity.mDataLanguageObj.getLanguage()
											.get(i).getId();
						} else {
							str = str
									+ ""
				...
				if (true) {
					hasChange = true;
					SetProfileByFieldLoader loader = new SetProfileByFieldLoader(
							"setField", activity, fieldName, str);
					activity.getRequestQueue().addRequest(loader);
			...
			OnClickListener listener = new OnClickListener() {
				...
				public void onClick(DialogInterface dialog2, int which) {
					...
					if (newDay != null && !newDay.equals(currentValue)) {
						hasChange = true;
						SetProfileByFieldLoader loader = new SetProfileByFieldLoader(
								"setField", activity, fieldName, "" + newDay);
						activity.getRequestQueue().addRequest(loader);
	...
	private void loadProfileInfo() {
		if (profileInfoObj != null) {
			...
			OakClubUtil.loadImageFromUrl(activity, url, avatar, "");

			if (!profileInfoObj.getVideo_link().equals("")) {
				url = OakClubUtil.getFullLinkVideo(activity,
						profileInfoObj.getVideo_link(),
						Constants.PHOTO_EXTENSION);
				OakClubUtil.loadImageFromUrl(activity, url, video, "");
			...
			String age = Integer.parseInt(yearNow.split("/")[2])
					- Integer.parseInt(profileInfoObj.getBirthday_date().split(
							"/")[2]) + "";
			Log.v("Age:", age);
			userNameTv.setText(profileInfoObj.getName() + ", " + age);
			userBriefInfoTv.setText(getWorkString(getWorkId(profileInfoObj
					.getWork()))
					+ ", "
	...
	private void solveDeleteVideo() {
		...
		tvMessage.setText(activity.getResources().getString(
				R.string.txt_delete_video_question));
	...
	public void doUploadPhoto(boolean isAvatar) {
		...
		if (filePath != null && !filePath.equals("")) {
			...
			if (checkFileMax(file, PHOTO_UPLOAD_MAX_SIZE)) {
				showDialogWarning(activity
						.getString(R.string.warning_profile_max_size_photo));
			} else {
	...
	public void doUploadVideo() {
		...
		if (filePath != null && !filePath.equals("")) {
			if (checkIfFileExistAndNotEmpty(filePath)) {
				new TranscdingBackground(activity, filePath, OakClubUtil
						.getFileStore(activity, "").getAbsolutePath()).execute();
			} else {
				showDialogWarning(activity
						.getString(R.string.warning_not_found_video));
			}
		}
	}
	...
	class UploadProfilePhotoLoader extends RequestUI {
		...
		public UploadProfilePhotoLoader(Object key, Activity activity,
			...
			if (!activity.isFinishing()) {
				...
				pdLoading.setMessage(activity
						.getString(R.string.txt_upload_photo));
		...
		public void executeUI(Exception ex) {
			...
			if (obj != null && obj.isStatus()) {
				...
				if (is_Avatar) {
					String imageUrl = OakClubUtil.getFullLink(activity,
							photoObj.getTweet_image_link());
					OakClubUtil.loadImageFromUrl(activity, imageUrl, avatar, "");
					profileInfoObj.setAvatar(photoObj.getTweet_image_link());
					activity.isChangedAvatar = true;
					arrListPhoto.add(0, photoObj);
				} else
					arrListPhoto.add(1, photoObj);
	...
	class UploadProfileVideoLoader extends RequestUI {
		...
		public UploadProfileVideoLoader(Object key, Activity activity, File file) {
			...
			if (!activity.isFinishing()) {
				pdLoading = new ProgressDialog(activity);
				pdLoading.setMessage(activity
						.getString(R.string.txt_upload_video));
				pdLoading.setCancelable(false);
				pdLoading.show();
			}
		}
		...
		if (SlidingMenuActivity.mDataLanguageObj != null)
			for (int i = 0; i < SlidingMenuActivity.mDataLanguageObj
				...
				Log.v("work-id", SlidingMenuActivity.mDataLanguageObj
						...
						+ " "
						+ SlidingMenuActivity.mDataLanguageObj.getWork_cate()
								.get(i).getCate_id());
				listString.add(SlidingMenuActivity.mDataLanguageObj
						.getWork_cate().get(i).getCate_name());
			}
	...
	private ArrayList<Integer> getWorkArrayListId() {
		...
		if (SlidingMenuActivity.mDataLanguageObj != null)
			for (int i = 0; i < SlidingMenuActivity.mDataLanguageObj
					.getWork_cate().size(); i++) {
				listId.add(SlidingMenuActivity.mDataLanguageObj.getWork_cate()
						.get(i).getCate_id());
			}
		return listId;
	}
	...
	private ArrayList<Integer> getEthnicityArrayListId() {
		...
		if (SlidingMenuActivity.mDataLanguageObj != null)
			for (int i = 0; i < SlidingMenuActivity.mDataLanguageObj
					.getEthnicity().size(); i++) {
				listId.add(SlidingMenuActivity.mDataLanguageObj.getEthnicity()
						.get(i).getId());
			}
		return listId;
	}
		...
		if (SlidingMenuActivity.mDataLanguageObj != null)
			for (int i = 0; i < SlidingMenuActivity.mDataLanguageObj
				...
				Log.v("relation-id", SlidingMenuActivity.mDataLanguageObj
						...
						+ " "
						+ SlidingMenuActivity.mDataLanguageObj
								.getRelationship_status().get(i)
								.getRel_status_id());
				listString.add(SlidingMenuActivity.mDataLanguageObj
						.getRelationship_status().get(i).getRel_text());
			}
	...
	public class TranscdingBackground extends
		...
		public TranscdingBackground(Activity act, String inputFile,
				String outputFile) {
			_act = act;
		...
		protected void onPreExecute() {
			if (!_act.isFinishing()) {
				...
				pdLoading.setMessage(_act
						.getString(R.string.txt_compressing_video));
				pdLoading.setCancelable(false);
				pdLoading.show();
			}
		}
		...
		protected Integer doInBackground(String... paths) {
			...
			String[] complexCommand = { "ffmpeg", "-y", "-i", inputFile,
					"-strict", "experimental", "-crf", "30", "-preset",
					"ultrafast", "-acodec", "aac", "-ar", "44100", "-ac", "2",
					"-b:a", "96k", "-vcodec", "libx264", "-r", "25", "-b:v",
					"500k", "-s", "800x480", "-f", "flv",
					outputFile + "/video.flv" };

			LoadJNI vk = new LoadJNI();

			try {
				vk.run(complexCommand, outputFile,
		...
		protected void onPostExecute(Integer result) {
			...
			if (checkFileMax(file, VIDEO_UPLOAD_MAX_SIZE)) {
				showDialogWarning(activity
						.getString(R.string.warning_profile_max_size_video));
			} else {
	...
	private static boolean checkIfFileExistAndNotEmpty(String fullFileName) {
		...
		Log.d(Constants.TAG, fullFileName + " length in bytes: "
				+ lengthInBytes);
