	...
	void consume(Purchase itemInfo) throws IabException {
		...
		if (!itemInfo.itemType.equals(ITEM_TYPE_INAPP)) {
			throw new IabException(IABHELPER_INVALID_CONSUMPTION,
					"Items of type '" + itemInfo.itemType
							+ "' can't be consumed.");
		}
...
//					+ mAsyncOperation + ") is in progress.");
	...
	public interface GetSkuFinishedListener {
		public void onGetSkuFinishedListener(ArrayList<SkuDetails> skuList);
	}

	public void getAllSKU(String itmeType, GetSkuFinishedListener listener) {
		ArrayList<String> skuList = new ArrayList<String>();
		ArrayList<SkuDetails> result = null;
		for (String id : Constants.PRODUCT_IDS) {
			skuList.add(id);
		}
		Bundle querySkus = new Bundle();
		...
		if (skuList.size() != 0) {
			...
			try {
				skuDetails = mService.getSkuDetails(3,
						mContext.getPackageName(), itmeType, querySkus);
				if(skuDetails != null){
					int response = skuDetails.getInt(RESPONSE_CODE);
					if (response == 0) {
						result = new ArrayList<SkuDetails>();
						if (skuDetails.containsKey(RESPONSE_GET_SKU_DETAILS_LIST)) {
							ArrayList<String> responseList = skuDetails
									.getStringArrayList(RESPONSE_GET_SKU_DETAILS_LIST);
							...
							for (String thisResponse : responseList) {
								SkuDetails d;
								try {
									d = new SkuDetails(itmeType, thisResponse);
									result.add(d);
								} catch (JSONException e) {
								}
							}
		...
		if (listener != null) {
			listener.onGetSkuFinishedListener(result);
		}
	}
}
