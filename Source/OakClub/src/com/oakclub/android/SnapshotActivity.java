...
public class SnapshotActivity extends SlidingMenuActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentViewX(R.layout.activity_snapshot);
		
		
		SharedPreferences pref = getSharedPreferences(
				"oakclub_pref", MODE_PRIVATE);
		...
		urlAvatar = OakClubUtil.getFullLink(this, ProfileSettingFragment.profileInfoObj.getAvatar());
		init(START_RECORD);
		
	}

	protected void init(int start) {
		contentSnapshot = 1;

		isFirst = true;
		widthScreen = (int) OakClubUtil.getWidthScreen(this);
	...
	public void showTutorialActivity() {
		intent = new Intent(SnapshotActivity.this,
	...
	private OnTouchListener onSlideTouch = new OnTouchListener() {
		...
		public boolean onTouch(final View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE: {
				x = event.getRawX();
				y = event.getRawY();
				Log.v("move: X _Y", x + "___" + y);
				tempX = (int) (x - dx);
				tempY = (int) (y - dy);
				parms.leftMargin = (int) (tempX);
				if (Math.abs(tempY) <= OakClubUtil
						.getWidthScreen(SnapshotActivity.this))
			...
			case MotionEvent.ACTION_UP: {
				...
				if (Math.abs(tempX) > widthScreen / 4) {
	                ...
	                if(tempX>0 && isFirstLike){
	                    String title = getString(R.string.like_string) + "?";
	                    String content = getString(R.string.snapshot_popup_like_info) + " " + target_name;
	                    showDialogFirst(title, content, true, true);
	                }
	                if(tempX<0 && isFirstNope){
	                    String title = getString(R.string.snapshot_not_interested) + "?";
	                    String content = getString(R.string.snapshot_popup_dislike_info) + " " + target_name;
					...
					isAction = false;
					Log.v("TempX__TempY", tempX + "____" + tempY);
        ...
        translate.setAnimationListener(new AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                SnapshotEvent snapEvent = new SnapshotEvent(
                        Constants.SET_FAVORITE, SnapshotActivity.this,
                        proId, action);
                getRequestQueue().addRequest(snapEvent);
		...
		if (!isAction) {
			...
			fadeOut.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					...
					boolean isLike = arrayListSnapshot.get(0).getIs_like();
					String url = OakClubUtil.getFullLink(
							getApplicationContext(), arrayListSnapshot.get(0)
									.getAvatar());
					nameProfile = arrayListSnapshot.get(0).getName();
                    avaProfile = arrayListSnapshot.get(0).getAvatar();
					arrayListSnapshot.remove(0);
	...
	private void setLikeSnapshot(String name){
	    ...
	    {
            String title = getString(R.string.like_string) + "?";
            String content = getString(R.string.snapshot_popup_like_info) + " " + name;
	...
	private void showDialogFirst(String title, String content, final boolean isDrag, boolean isLike){
        ...
        if(isLike){
            ...
            isFirstLike = false;
            tvTitle.setTextColor(getResources().getColor(R.color.dialog_snapshot_like));
            btActive.setText(getString(R.string.like_string));
            btActive.setBackgroundResource(R.drawable.dialog_snapshot_like);
            btActive.setTextColor(getResources().getColor(R.color.dialog_snapshot_text_like));
            ...
            isFirstNope = false;
            tvTitle.setTextColor(getResources().getColor(R.color.dialog_snapshot_pass));
            btActive.setText(getString(R.string.pass_string));
            btActive.setBackgroundResource(R.drawable.dialog_snapshot_pass);
            btActive.setTextColor(getResources().getColor(R.color.dialog_snapshot_text_pass));
            btActive.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if(!isDrag)
        ...
        public void onClick(View v) {switch (v.getId()) {
            case R.id.popup_mutual_match_layout_btn_chat:
                ...
                SetViewMuatualEvent loader = new SetViewMuatualEvent(
                        Constants.SET_VIEW_MUTUAL_MATCH, SnapshotActivity.this,
                        profileId);
                getRequestQueue().addRequest(loader);
                intent = new Intent(
                        SnapshotActivity.this.getApplicationContext(),
                        ChatActivity.class);
	...
	private void swapData(int index) {
		if (isInternetAccess()) {
			Log.v("index - size:", index + " " + arrayListSnapshot.size());
			...
			AlertDialog.Builder builder;
			builder = new AlertDialog.Builder(SnapshotActivity.this);
			final AlertDialog dialog = builder.create();
			LayoutInflater inflater = LayoutInflater
					.from(SnapshotActivity.this);
	...
	private void changeImage(final int index) {
		final String imageUrl = OakClubUtil.getFullLink(this, arrayListSnapshot
				.get(index).getAvatar(), Constants.widthImage, Constants.heightImage, 1);
		Log.v("width - height", Constants.widthImage + " " + Constants.heightImage);
		Log.v("TAG", "TAG:string=" + arrayListSnapshot.get(index).getAvatar()
				+ ",imageFull=" + imageUrl);
		handler = new Handler();
		Log.v("contentSnapshot", contentSnapshot + "");
		Log.v("name Snapshot:", arrayListSnapshot.get(index).getName());
		switch (contentSnapshot) {
		...
		OakClubUtil.loadImageFromUrl(SnapshotActivity.this,imageUrl, imgAva);
//	    OakClubUtil.loadImageFromUrl(SnapshotActivity.this,imgAva, imageUrl);
	}
	...
	public void getListSnapshotData(int start) {
		...
		GetSnapShotLoader loader = new GetSnapShotLoader(
				Constants.GET_SNAPSHOT, SnapshotActivity.this,
	...
	private void showDialog(String url) {
		index = 1;
		dialogMutual = new Dialog(SnapshotActivity.this);
		...
		OakClubUtil.loadImageFromUrl(getApplicationContext(), ivwMutualMe,
				urlAvatar);
		OakClubUtil
				.loadImageFromUrl(getApplicationContext(), ivwMutualYou, url);
		txtMutualMatch
	...
	class SnapshotEvent extends RequestUI {
		...
		public void execute() throws Exception {
			resultEvent = oakClubApi.SetFavoriteInSnapshot(proId, numberSet);
		}
	...
	class SetViewMuatualEvent extends RequestUI {
		...
		@Override
		public void execute() throws Exception {
			resultEvent = oakClubApi.SetViewedMutualMatch(proId);
		}
	...
	class GetSnapShotLoader extends RequestUI {
		...
		@Override
		public void execute() throws Exception {
			obj = oakClubApi.getSnapShot(userId, startIndex, limitIndex);
		}
