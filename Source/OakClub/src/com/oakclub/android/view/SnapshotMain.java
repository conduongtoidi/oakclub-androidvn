...
public class SnapshotMain extends FrameLayout {
	...
	public SnapshotMain(Context context, SnapshotFragment snapshot,
			SnapshotData data) {
		super(context);
	...
	private void init() {
		...
		fltParams = new LayoutParams(LayoutParams.WRAP_CONTENT,
		...
		fltImage = (FrameLayout) view
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_flt_image);
		paddingView = (int) OakClubUtil.convertDpToPixel(20, getContext());
		fltImage.setPadding(paddingView, paddingView, paddingView, paddingView);
		fltParams = (LayoutParams) fltImage.getLayoutParams();
		rltInfo = (RelativeLayout) view
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_rlt_info);
		paddingView = (int) OakClubUtil.convertDpToPixel(10, getContext());
		rltInfo.setPadding(paddingView, paddingView, paddingView, paddingView);
		rltInfoRight = (RelativeLayout) rltInfo
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_rlt_info_right);
		rltParams = (RelativeLayout.LayoutParams) rltInfoRight
				.getLayoutParams();
		imgAvatar = (SmartImageView) fltImage
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_avatar);
		imgPlayVideo = (ImageButton) fltImage
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_imgPlayvideo);
		imgVip = (ImageView) fltImage
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_imgVip);
		imgMutualFriend = (ImageView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_mutual_friend);
		imgShareInterest = (ImageView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_shareinterest);
		imgVerified = (ImageView) rltInfo
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_photo_verified);
		imgPhoto = (ImageView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_photo);
		tvName = (TextView) rltInfo
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_tvw_info);
		tvNumFriend = (TextView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_tv_numFriend);
		tvMutualFriend = (TextView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_tvw_mutual_friend);
		tvShareInterest = (TextView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_tvw_shareinterest);
		tvCountPhoto = (TextView) rltInfoRight
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_tvw_photo);

		ivwLikeStamp = (ImageView) fltImage
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_like);
		ivwNopeStamp = (ImageView) fltImage
				.findViewById(R.id.activity_snapshot_flt_body_flt_content_ivw_nope);

		pref = getContext().getSharedPreferences(Constants.PREFERENCE_NAME, 0);
	...
	private void loadData(SnapshotData data) {
		...
		vto.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				fltParent.getViewTreeObserver().removeGlobalOnLayoutListener(
						this);
		...
		vto.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			@Override
			public void onGlobalLayout() {
				rltInfo.getViewTreeObserver()
						.removeGlobalOnLayoutListener(this);
				rltParams = new RelativeLayout.LayoutParams(imgMutualFriend
						.getMeasuredWidth() * 8, imgMutualFriend
						.getMeasuredWidth() * 8);
				...
				fltParams = new LayoutParams(fltImage.getMeasuredWidth(),
						fltImage.getMeasuredWidth());
				// fltParams.gravity=Gravity.BOTTOM;
				fltParams.bottomMargin = imgMutualFriend.getMeasuredHeight()
						+ tvMutualFriend.getMeasuredHeight() + paddingView;
				fltImage.setLayoutParams(fltParams);
				fltParams = new LayoutParams(LayoutParams.MATCH_PARENT,
						rltInfoRight.getMeasuredHeight());
				fltParams.gravity = Gravity.BOTTOM;
		...
		if (data != null) {
			...
			imageUrl = OakClubUtil.getFullLink(getContext(), data.getAvatar(),
					Constants.widthImage, Constants.heightImage, 1);
			imgAvatar.setBackgroundResource(R.drawable.logo_splashscreen);
			imgAvatar.setImageBitmap(null);
			OakClubUtil.loadImageFromUrl(getContext(), imageUrl, imgAvatar, Constants.SNAPSHOT_FOLDER);
			if (!urlVideo.equals("")) {
			...
			if (!data.isIs_vip()) {
				if (numFriend.equals("0"))
					imgMutualFriend
							.setBackgroundResource(R.drawable.ico_mutualfriend_disable);
				else
					imgMutualFriend
							.setBackgroundResource(R.drawable.ico_mutualfriend_normal);

				if (numLikePerson.equals("0"))
					imgShareInterest
							.setBackgroundResource(R.drawable.ico_sharedinterest_disable);
				else
					imgShareInterest
							.setBackgroundResource(R.drawable.ico_sharedinterest_normal);
				...
				if (numFriend.equals("0")) {
					tvMutualFriend.setTextColor(getResources().getColor(
							R.color.text_vip_disable));
					imgMutualFriend
							.setBackgroundResource(R.drawable.multual_friends_inactivated);
				} else {
					tvMutualFriend.setTextColor(getResources().getColor(
							R.color.text_vip));
					imgMutualFriend
							.setBackgroundResource(R.drawable.multual_friends_activated);
				}
				if (numLikePerson.equals("0")) {
					tvShareInterest.setTextColor(getResources().getColor(
							R.color.text_vip_disable));
					imgShareInterest
							.setBackgroundResource(R.drawable.interests_inactivated);
				} else {
					tvShareInterest.setTextColor(getResources().getColor(
							R.color.text_vip));
					imgShareInterest
							.setBackgroundResource(R.drawable.interests_activated);
				}
				if (numPicture.equals("0")) {
					tvCountPhoto.setTextColor(getResources().getColor(
							R.color.text_vip_disable));
	...
	public void showDialogMutualMatch() {
		final Dialog dialogMutual = new Dialog(getContext());// ,
																// android.R.style.Theme_Translucent);
		dialogMutual.requestWindowFeature(Window.FEATURE_NO_TITLE);

		WindowManager.LayoutParams params = dialogMutual.getWindow()
				.getAttributes();
		...
		TextViewWithFont tvTitle = (TextViewWithFont) view
				.findViewById(R.id.tvw_text);
		...
		TextViewWithFont txtMutualMatch = (TextViewWithFont) view
				.findViewById(R.id.tvw_text_notice);
		...
		OakClubUtil.loadImageFromUrl(getContext(), snapshot.urlAvatar,
				ivwMutualMe, Constants.SNAPSHOT_FOLDER);
		OakClubUtil.loadImageFromUrl(getContext(), imageUrl, ivwMutualYou, Constants.SNAPSHOT_FOLDER);
		txtMutualMatch
	...
	private void showDialogFirst(final boolean isDrag, final boolean isLike) {
		...
		if (isLike) {
			...
			content = getContext().getString(R.string.snapshot_popup_like_info)
					+ " " + nameProfile;
		} else {
			title = getContext().getString(R.string.snapshot_not_interested)
					+ "?";
			content = getContext().getString(
					R.string.snapshot_popup_dislike_info)
					+ " " + nameProfile;
		}
		...
		LayoutInflater inflater = LayoutInflater.from(getContext());
		View layout = inflater.inflate(R.layout.dialog_active_snapshot, null);
		dialog.setView(layout, 0, 0, 0, 0);
		dialog.getWindow().setBackgroundDrawable(
				new ColorDrawable(android.graphics.Color.TRANSPARENT));
		TextView tvTitle = (TextView) layout
				.findViewById(R.id.dialog_active_snapshot_tvtitle);
		tvTitle.setText(title);
		TextView tvContent = (TextView) layout
				.findViewById(R.id.dialog_active_snapshot_tvcontent);
		tvContent.setText(content);
		Button btActive = (Button) layout
				.findViewById(R.id.dialog_active_snapshot_btActive);
		...
		Button btCancel = (Button) layout
				.findViewById(R.id.dialog_active_snapshot_btCancel);
	...
	private void dragSnapshotAnimation() {
		...
		translate.setAnimationListener(new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				snapshot.setFavoriteSnapshot(profileId, isLike,
						data.getIs_like());
				if (snapshot.fltBody.getChildCount() == 1)
					snapshot.fltBody.removeViewAt(0);
				else
					snapshot.fltBody.removeViewAt(1);
	...
	private void returnSnapshot(float x, float y) {
		...
		RotateAnimation rotateAnim = new RotateAnimation(0, -angle, 0, 0, 0, 0);
	...
	public void setPressButtonAction(String action) {
		isFirstLike = pref.getBoolean(Constants.PREFERENCE_SHOW_LIKE_DIALOG,
				true);
		isFirstNope = pref.getBoolean(Constants.PREFERENCE_SHOW_NOPE_DIALOG,
				true);
		if (Constants.ACTION_LIKE.equals(action) && isFirstLike) {
			...
			buttonSnapshotAnimation(action);
		ImageLoader imgLoader = new ImageLoader(getContext(), Constants.OTHER_PROFILE_FOLDER);
		imgLoader.clearCache();
	...
	private void buttonSnapshotAnimation(final String action) {
		...
		if (Constants.ACTION_LIKE.equals(action)) {
			translateAnim = new TranslateAnimation(Animation.ABSOLUTE,
					widthScreen, Animation.ABSOLUTE, Animation.ABSOLUTE);
			translateAnim.setStartOffset(TIMER);
			rotateAnim = new RotateAnimation(RotateAnimation.ABSOLUTE,
					-getAngle(-widthScreen), RotateAnimation.RELATIVE_TO_SELF,
					0.5f, RotateAnimation.RELATIVE_TO_SELF, 0.5f);
			rotateAnim.setStartOffset(TIMER);

			ivwLikeStamp.setAlpha(1.0f);
			ivwNopeStamp.setAlpha(0.0f);
			animButton = new ScaleAnimation(1.5f, 1f, 1.5f, 1f,
					Animation.ABSOLUTE, ivwLikeStamp.getWidth() / 2,
					Animation.ABSOLUTE, ivwLikeStamp.getHeight() / 2);
			...
			animButton = new ScaleAnimation(1.5f, 1f, 1.5f, 1f,
					Animation.ABSOLUTE, ivwNopeStamp.getWidth() / 2,
					Animation.ABSOLUTE, ivwNopeStamp.getHeight() / 2);
			animButton.setDuration(TIMER);
		...
		fadeOut.setAnimationListener(new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				setLoadingAnim(false);
				snapshot.setFavoriteSnapshot(profileId, action,
						data.getIs_like());
				if (snapshot.fltBody.getChildCount() == 1)
					snapshot.fltBody.removeViewAt(0);
				else
