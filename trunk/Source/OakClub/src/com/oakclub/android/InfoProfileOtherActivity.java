...
public class InfoProfileOtherActivity extends OakClubBaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		profile_info_other_image_height = (int) OakClubUtil
				.getHeightScreen(getApplicationContext()) * 2 / 3;
		init();
	...
	private void SlideToDown(RelativeLayout rl, int duration) {
		...
		slide = new TranslateAnimation(rl.getX(), rl.getX(), rl.getY()
				- rl.getMeasuredHeight(), rl.getY());
	...
	private void init() {
		...
		imageCache = (ArrayList<ListPhotoReturnDataItemObject>) snapShotData
				.getPhotos();
		...
		pager.setOnPageChangeListener(new OnPageChangeListener() {
			...
			public void onPageSelected(int arg0) {
				if (arg0 + 1 <= imageCache.size())
					...
					tvwNumCapture.setText(arg0 + "/" + imageCache.size());
			}
		...
		tvwNumCapture.setText((pager.getCurrentItem() + 1) + "/"
				+ imageCache.size());
	...
	private OnClickListener zoomEvent = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (!isZoom) {
				...
				int rsNaviHeight = getApplicationContext().getResources()
						.getIdentifier("navigation_bar_height", "dimen",
								"android");
				int rsStatusHeight = getApplicationContext().getResources()
				...
				rltMain.measure(MeasureSpec.UNSPECIFIED,
						MeasureSpec.UNSPECIFIED);
				Log.v("rltHeight", rltMain.getHeight() + "");
				Log.v("Height Srceen",
						OakClubUtil.getHeightScreen(getApplicationContext())
								+ "");
				Log.v("NavHeight", NaviHeight + "");
				Log.v("StatusHeight", StatusHeight + "");
				animation = new ResizeAnimation(rltMain, rltMain.getHeight(),
						(int) OakClubUtil
						...
						.setAnimationListener(new Animation.AnimationListener() {
							...
							public void onAnimationEnd(Animation animation) {
								Log.v("rltHeightEndABC", rltMain.getHeight()
										+ "");
								RelativeLayout.LayoutParams layout = new android.widget.RelativeLayout.LayoutParams(
										RelativeLayout.LayoutParams.MATCH_PARENT,
										RelativeLayout.LayoutParams.MATCH_PARENT);
								rltMain.setLayoutParams(layout);

								FrameLayout.LayoutParams layoutPager = new android.widget.FrameLayout.LayoutParams(
				...
				Animation fadeOut = AnimationUtils.loadAnimation(
						...
						R.anim.anim_fadeout_content_info);
	...
	private String getWorkString(int id) {
		...
		if (SlidingMenuActivity.mDataLanguageObj != null)
			for (int i = 0; i < SlidingMenuActivity.mDataLanguageObj
					.getWork_cate().size(); i++) {
				if (SlidingMenuActivity.mDataLanguageObj.getWork_cate().get(i)
						.getCate_id() == id) {
					return SlidingMenuActivity.mDataLanguageObj.getWork_cate()
							.get(i).getCate_name();
				}
			}
		return str;
	...
	private OnClickListener eventClickButton = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.activity_profile_info_other_header_fltSnapshot_imgPlayvideo:
				String videoUrl = OakClubUtil.getFullLinkVideo(
						InfoProfileOtherActivity.this, urlVideo,
						Constants.VIDEO_EXTENSION);
				intent = new Intent(InfoProfileOtherActivity.this,
	...
	private void solveEditBtn() {
		if (!InfoProfileOtherActivity.this.isFinishing()) {
			...
			for (int i = 0; i < stringList.length; i++) {
				...
				if (radio.getText().equals(
						getString(R.string.txt_report_user_other))) {
	...
	private void showBlockConfirmDialog(String tvQuestion, String btName,
				...
				public void onClick(View arg0) {
					if (isReport) {
						...
						BlockUserRequest loader = new BlockUserRequest(
								"blockUser", InfoProfileOtherActivity.this,
								profileId);
						getRequestQueue().addRequest(loader);
					}
					dialog.dismiss();
	...
	protected void assignInfo() {
		if (snapShotData != null) {
			...
			HashMap<String, String> t = ParseDataProfileInfo
					.getDataOtherProfile(getApplicationContext(), snapShotData);
			int age = snapShotData.getAge();
			tvwName.setText(OakClubUtil.getFirstName(snapShotData.getName())
					+ ", " + age);
			if (age <= 1)
				Log.e("Error", "Error:InfoProfileOtherActivity:age is wrong");
			tvwAge.setText(age + "");
			...
			if (snapShotData.getAbout_me() != null
					&& !snapShotData.getAbout_me().isEmpty()) {
			...
			if (snapShotData.getShare_interests() != null
					&& !snapShotData.getShare_interests().isEmpty()) {

				vpShareInterests.setListForView(snapShotData
						.getShare_interests());
			...
			if (snapShotData.getMutual_friends() != null
					&& !snapShotData.getMutual_friends().isEmpty()) {
				vpMutualFriends
						.setListForView(snapShotData.getMutual_friends());
			...
			if (snapShotData.getLocation_name() != null
					&& snapShotData.getLocation_name() != null) {
				tvLocation.setText(snapShotData.getLocation_name());
			} else {
				tvLocation.setVisibility(View.GONE);
				imgLocation.setVisibility(View.GONE);
			}
			if (snapShotData.getHometown_name() != null
					&& snapShotData.getHometown_name() != null) {
				tvHome.setText(snapShotData.getHometown_name());
			} else {
				tvHome.setVisibility(View.GONE);
				imgHome.setVisibility(View.GONE);
			}
			if (snapShotData.getBirthday_date() != null
					&& !snapShotData.getBirthday_date().isEmpty()) {
				tvBirthday.setText(snapShotData.getBirthday_date());
			} else {
				tvBirthday.setVisibility(View.GONE);
				imgBirthday.setVisibility(View.GONE);
			}
			if (snapShotData.getSchool() != null
					&& !snapShotData.getSchool().isEmpty()) {
			...
			if (snapShotData.getInterested() > 0) {
				tvInterested_In.setText(ParseDataProfileInfo.getInterested(
						getApplicationContext(), snapShotData.getInterested()));
			} else {
	...
	private class ImagePagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup view, int position) {
			FrameLayout fltImage = new FrameLayout(
					InfoProfileOtherActivity.this);

			android.widget.FrameLayout.LayoutParams params;
			if (position < imageCache.size()) {
				SmartImageView imageView = new SmartImageView(
						InfoProfileOtherActivity.this);
				...
				String imageUrl = OakClubUtil.getFullLink(
						InfoProfileOtherActivity.this, imageCache.get(position)
								.getTweet_image_link(), Constants.widthImage,
						Constants.heightImage, 1);
				OakClubUtil.loadImageFromUrl(InfoProfileOtherActivity.this,
						imageUrl, imageView, Constants.OTHER_PROFILE_FOLDER);
				...
				if (!urlVideo.equals("")) {
					...
					params.gravity = Gravity.CENTER;
					imageView.setLayoutParams(params);
					String imageUrl = OakClubUtil.getFullLinkVideo(
							InfoProfileOtherActivity.this, urlVideo,
					...
					ImageView imageViewVideo = new ImageView(
							InfoProfileOtherActivity.this);
					params = new android.widget.FrameLayout.LayoutParams(
					...
					imageViewVideo
							.setBackgroundResource(R.drawable.large_video_btn);
					// imageViewVideo.setLayoutParams(new
	...
	private class BlockUserRequest extends RequestUI {
		...
		public void executeUI(Exception ex) {
			...
			// getApplicationContext().getSharedPreferences(Constants.PREFERENCE_NAME,
			// 0);
			// Editor editor = pref.edit();
			// editor.putBoolean(Constants.IS_LOAD_CHAT_AGAIN, true);
			// editor.commit();
