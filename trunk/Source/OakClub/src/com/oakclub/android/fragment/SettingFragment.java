...
public class SettingFragment {
	...
	public void initSetting() {
		...
		seekBar = new RangeSeekBar<Integer>(Constants.MIN_SEARCH_AGE,
				Constants.MAX_SEARCH_AGE, activity);
		...
		stringList = activity.getResources().getStringArray(
	...
	public void init() {
		...
		if (Constants.settingObject == null) {
			...
			seekBar.setNormalizedMinValue(((double) settingObject.getAge_from() - Constants.MIN_SEARCH_AGE)
					/ seekBar.getAbsoluteMaxValue());
			seekBar.setNormalizedMaxValue(((double) settingObject.getAge_to() - Constants.MIN_SEARCH_AGE)
					/ seekBar.getAbsoluteMaxValue());
			tvAgeLimit.setText("" + settingObject.getAge_from() + " "
					+ activity.getString(R.string.txt_age_from) + " "
					+ settingObject.getAge_to() + " "
					+ activity.getString(R.string.txt_age_to));
			int distance = (int) ((settingObject.getRange() - 50) / 50);
			distanceSeekbar.setProgress(distance);

			maxDistance = settingObject.getRange();
			if (maxDistance < 50)
				tvDistance.setText(activity
						.getString(R.string.msg_people_around_sub_1)
						+ " "
						+ "50km");
				...
				tvDistance.setText(activity
						.getString(R.string.msg_people_around_sub_1)
						+ " "
						+ activity.getString(R.string.txt_distance_world));
			else if (maxDistance >= 350)
				tvDistance.setText(activity
						.getString(R.string.msg_people_around_sub_1)
						+ " "
						+ activity.getString(R.string.txt_distance_country));
		...
		if (!Constants.isChangedSetting) {
			LoadAccountSettingsLoader settingLoader = new LoadAccountSettingsLoader(
					"getAccountSetting", activity);
		...
		public void executeUI(Exception ex) {
			...
			if (snapshotObj != null && snapshotObj.getData() != null) {
				if (Constants.settingObject == null) {
					...
					cbIncludeFriends.setChecked(snapshotObj.getData()
							.isInclude_friend());
					seekBar.setNormalizedMinValue(((double) snapshotObj
							.getData().getAge_from() - Constants.MIN_SEARCH_AGE)
							/ seekBar.getAbsoluteMaxValue());
					seekBar.setNormalizedMaxValue(((double) snapshotObj
							.getData().getAge_to() - Constants.MIN_SEARCH_AGE)
							/ seekBar.getAbsoluteMaxValue());
					tvAgeLimit.setText("" + snapshotObj.getData().getAge_from()
							+ " " + activity.getString(R.string.txt_age_from)
							+ " " + snapshotObj.getData().getAge_to() + " "
							+ activity.getString(R.string.txt_age_to));
					...
					if (maxDistance < 50)
						...
						tvDistance
								.setText(activity
										...
										+ " "
										+ activity
												.getString(R.string.txt_distance_world));
					else if (maxDistance >= 350)
						tvDistance
								.setText(activity
										.getString(R.string.msg_people_around_sub_1)
										+ " "
										+ activity
												.getString(R.string.txt_distance_country));
				...
				settingObject.setInclude_friends(snapshotObj.getData()
						.isInclude_friend());
	...
	public void showLanguagueDialog(final int currentId) {
		if (!activity.isFinishing()) {
			...
			final AlertDialog dialog = builder.create();// new
														// Dialog(ProfileSettingActivity.activity);
			...
			for (int i = 0; i < stringList.length; i++) {
				...
				RadioButton radio;
				// if(stringList[i] ==
				// activity.getString(R.string.txt_language_ar)){
				// radio = new RadioButton(activity);
				// radio.setBackgroundResource(R.drawable.radiogroup_selector2);
				// params.gravity = Gravity.LEFT|Gravity.CENTER_VERTICAL;
				// }
				// else
				radio = new RadioButtonCustom(activity, activity.getResources()
						.getDrawable(R.drawable.radiogroup_selector2));

				// RadioButtonCustom radio = new
				// RadioButtonCustom(activity,activity.getResources().getDrawable(R.drawable.radiogroup_selector2));
				...
				public void onClick(View arg0) {
					...
					if (currentId != selectedId) {
						if (selectedId < Constants.LANGUAGE_LOCALE.length)
							...
							setLocale(Constants.LANGUAGE_LOCALE[0]);
						...
						Constants.country = activity.getResources()
						...
						UpdateLanguage loader = new UpdateLanguage(
								"updateLanguageApp", activity,
								Constants.country);
	...
	private OnRangeSeekBarChangeListener<Integer> evenRangeSeekBar = new OnRangeSeekBarChangeListener<Integer>() {
		...
		public void onRangeSeekBarValuesChanged(RangeSeekBar<?> bar,
			...
			tvAgeLimit.setText("" + minValue + " "
					+ activity.getString(R.string.txt_age_from) + " "
					+ maxValue + " " + activity.getString(R.string.txt_age_to));
	...
	private OnClickListener evenClick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.linear_contact_us:
				...
				intent.putExtra(Intent.EXTRA_EMAIL, new String[] { activity
						.getString(R.string.mail_server_address) });
				intent.putExtra(Intent.EXTRA_SUBJECT,
						activity.getString(R.string.txt_subject_mail_contact));
